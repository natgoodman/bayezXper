...
dat_effit=
  function(n=param(n),m=param(m),prop.true=param(prop.true),d0=param(d0),
           mean.true=param(mean.true),sd.true=param(sd.true),
           mean.false=param(mean.false),sd.false=param(sd.false),
           ...) {
    ...
    sim_effit('lo_norm',n=n.lo[1],m=m,d0=d0,1,mean.true,sd.true,...);
    if (length(n.lo)>1) sim_effit('lo_norm',n=n.lo[1],m=m,d0=d0,1,mean.true,sd.true,...);
    sim_effit('hi_norm',n=n.hi,m=m,d0=d0,1,mean.true,sd.true,...);
    ## mixtures
    mean.mix=c(mean.true,mean.false);
    sd.mix=c(sd.true,sd.false);
    sim_effit('lo_lo',n=n.lo[1],m=m,d0=d0,prop.lo,mean.mix,sd.mix);
    if (length(n.lo)>1) sim_effit('lo_lo',n=n.lo[2],m=m,d0=d0,prop.lo,mean.mix,sd.mix);
    sim_effit('hi_lo',n=n.hi,m=m,d0=d0,prop.lo,mean.mix,sd.mix);
    sim_effit('lo_hi',n=n.lo[1],m=m,d0=d0,prop.hi,mean.mix,sd.mix);
    if (length(n.lo)>1) sim_effit('lo_hi',n=n.lo[2],m=m,d0=d0,prop.hi,mean.mix,sd.mix);
    sim_effit('hi_hi',n=n.hi,m=m,d0=d0,prop.hi,mean.mix,sd.mix);
    invisible();
  }
## use norMix so same code will work for norm and mixture
sim_effit=function(id,n,m,d0,prop.true,mean.mix,sd.mix,...) {
  w=if(prop.true==1) 1 else c(prop.true,1-prop.true);
  sim=dosim(id,n,m,d0,d.gen=rnorMix,d.args=list(obj=norMix(mu=mean.mix,sigma=sd.mix,w=w)),...);
  invisible(sim);
}
vrnorm=Vectorize(rnorm,"mean");
dosim=
  function(id,n,m,d0,d.gen,d.args,tol=param(tol),m1=param(m1),mmax=param(mmax)) {
    param(verbose,debug);
    if (verbose) {
